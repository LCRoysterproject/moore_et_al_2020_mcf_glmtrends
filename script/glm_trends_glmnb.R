...
modnames = c("period", "period + locality", "period * locality", "period + site", 
             "period * site", "period + locality + site", "period + locality * site", 
             "period * locality + site", "period * site + locality")
...
testCK = ggpredict(glmm8, 
                   terms = c("period", "site", "locality[CK]", "tran_length[33.23]"), 
                   type = c("fe"), data = d2)
newCK = data.frame(x = testCK$x, 
                   predicted = testCK$predicted, site = rep(c("I", "N", "O"), each = 1, times = 10),
                   conf.low = testCK$conf.low, conf.high = testCK$conf.high)
testCR = ggpredict(glmm8, 
                   terms = c("period", "site", "locality[CR]", "tran_length[33.23]"), 
                   type = c("fe"), data = d2)
newCR = data.frame(x = testCR$x, 
                   predicted = testCR$predicted, site = rep(c("I", "N", "O"), each = 1, times = 10), 
                   conf.low = testCR$conf.low, conf.high = testCR$conf.high)
testHB = ggpredict(glmm8, 
                   terms = c("period", "site", "locality[HB]", "tran_length[33.23]"), 
                   type = c("fe"), data = d2)
newHB = data.frame(x = testHB$x, 
                   predicted = testHB$predicted, site = rep(c("I", "N", "O"), each = 1, times = 10),
                   conf.low = testHB$conf.low, conf.high = testHB$conf.high)
testLC = ggpredict(glmm8, 
                   terms = c("period", "site", "locality[LC]", "tran_length[33.23]"), 
                   type = c("fe"), data = d2)
newLC = data.frame(x = testLC$x, 
                   predicted = testLC$predicted, site = rep(c("I", "N", "O"), each = 1, times = 10), 
                   conf.low = testLC$conf.low, conf.high = testLC$conf.high)
...
# save_plot("data_output/big_bend_trends/graphs/Fig3GLMNB_glmm8_final.tiff", pm2, 
          # dpi = 300, base_width = 10, base_height = 7)
...
d3 <- merge(d2, dis_mean_year[,c("Year", "Discharge")], 
            by.x = "year", by.y = "Year", all.x = TRUE)
names(d3)[length(names(d3))] <- "AD"
d3 <- merge(d3, dis_mean_yearW[,c("YearW", "DischargeW")], 
            by.x = "year", by.y = "YearW", all.x = TRUE)
names(d3)[length(names(d3))] <- "AD_W"

d4 <- merge(d3, dis_mean_year[,c("YearLag1", "Discharge")], 
            by.x = "year", by.y = "YearLag1", all.x = TRUE)
names(d4)[length(names(d4))] <- "ADLag1"
d4 <- merge(d4, dis_mean_yearW[,c("YearLag1W", "DischargeW")], 
            by.x = "year", by.y = "YearLag1W", all.x = TRUE)
names(d4)[length(names(d4))] <- "ADLag1_W"

d5 <- merge(d4, dis_mean_year[,c("YearLag2", "Discharge")], 
            by.x = "year", by.y = "YearLag2", all.x = TRUE)
names(d5)[length(names(d5))] <- "ADLag2"
d5 <- merge(d5, dis_mean_yearW[,c("YearLag2W", "DischargeW")],
            by.x = "year", by.y = "YearLag2W", all.x = TRUE)
names(d5)[length(names(d5))] <- "ADLag2_W"

d6 <- merge(d5, dis_total_year[,c("Year", "Discharge")], 
            by.x = "year", by.y = "Year", all.x = TRUE)
names(d6)[length(names(d6))] <- "TD"
d6 <- merge(d6, dis_total_yearW[,c("YearW", "DischargeW")],
            by.x = "year", by.y = "YearW", all.x = TRUE)
names(d6)[length(names(d6))] <- "TD_W"

d7 <- merge(d6, dis_total_year[,c("YearLag1", "Discharge")], 
            by.x = "year", by.y = "YearLag1", all.x = TRUE)
names(d7)[length(names(d7))] <- "TDLag1"
d7 <- merge(d7, dis_total_yearW[,c("YearLag1W", "DischargeW")], 
            by.x = "year", by.y = "YearLag1W", all.x = TRUE)
names(d7)[length(names(d7))] <- "TDLag1_W"

d8 <- merge(d7, dis_total_year[,c("YearLag2", "Discharge")], 
            by.x = "year", by.y = "YearLag2", all.x = TRUE)
names(d8)[length(names(d8))] <- "TDLag2"
d8 <- merge(d8, dis_total_yearW[,c("YearLag2W", "DischargeW")],
            by.x = "year", by.y = "YearLag2W", all.x = TRUE)
...
cand.set = list(glmm8, glmm11, glmm12, glmm13, glmm14, glmm15, glmm16, 
                glmm17, glmm18, glmm19, glmm20, glmm21, glmm22, glmm23)
modnames = c("Base", "AD", "ADLag1", "ADLag2", "TD", "TDLag1", "TDLag2",
             "Landings", "LandingsLag1", "LandingsLag2", "Trips", 
             "TripsLag1", "TripsLag2", "Harvest")
aictab(cand.set, modnames, second.ord = FALSE) #model selection table with AIC
...
glmm12_new <- glmmTMB(count_live ~ period * locality + site + ADLag1sc + offset(tran_length), 
                      data = new.dat, family = 'nbinom1') 
...
# tiff("data_output/big_bend_trends/graphs/Fig2SPvalDensity_final.tiff", res = 300, 
     # units = 'px', height = 3200, width = 3200)
